...
class Github_basic(commands.Cog):
    ...
    @nextcord.slash_command(name="git-repo-info", description="Display repository information")
    async def repoinfo(self, interaction: nextcord.Interaction, repo: str):
        """Display information about a specific GitHub repository."""
        self.github_token=await self.DB["UserInfo"].find_one({"user":interaction.user.name})["Token"]
